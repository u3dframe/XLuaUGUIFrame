Shader "S_E/EffectCombine"
{
	Properties
	{
		_RenderMode("Rendering Mode", Float) = 0
		[HideInInspector] _TwoSide("Two Side", Float) = 2
		[HideInInspector] _Cutout("Cutoff", Range(0,1)) = 0.1
		[HideInInspector] _BlendMode("Blend Mode", Float) = 0
		[HideInInspector] _SrcBlend("Src Blend", Float) = 1
		[HideInInspector] _DstBlend("Dst Blend", Float) = 0
		[HideInInspector] _SrcAlpha("Src Alpha", Float) = 0
		[HideInInspector] _DstAlpha("Dst Alpha", Float) = 0
	...
	SubShader
	{
		Tags{ "IgnoreProjector" = "True" "RenderType"="Opaque" "LightMode"="Always" }
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
			...
			#ifdef USE_FLUID_LIGHT				
				fixed4 c = 	tex2D(_LightTex, i.lightUV);
				fixed3 light = c.rgb * _LightStrength * c.a;
				light *= _LightColor;
				...
				if(_Amount > 0)
				{
					fixed4 mask = tex2D(_DissolveMask, i.texcoord);
	...
	CustomEditor "CoreEditor.Drawer.EffectCombineShaderGUI"
}
