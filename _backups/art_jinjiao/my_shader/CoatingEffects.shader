...
Shader "Custom/CoatingEffects" {
	Properties {
		...
		_HDRColor ("HDR颜色", COLOR) = (1,1,1,1)
	   	...
	   	_Color("Color (色调)", COLOR) = (1,1,1,1)
		[Enum (ORG,0,STATUE,1)] _CCType ("Curr Type([石化:1)", int) = 0
		_StatueTex("Statue Tex (石化图片)", 2D) = "white" {}
		_StatueDegree("Statue Degree (石化程度)", Range(0, 1)) = 1
		_StatueBrightness("Statue Brightness (石化亮度)", range(0.5, 8)) = 1.6
	}
	...
	CGINCLUDE
		...
		int _CCType;
		uniform sampler2D _StatueTex;
		uniform float4 _StatueTex_ST;
		uniform float _StatueDegree;
		uniform float _StatueBrightness;

		float4 frag_statue(float4 col,float2 uv) : Color{
			if(1 != _CCType){
				return col;
			}
			// col.a = 1;
			float4 colStatue = tex2D(_StatueTex, TRANSFORM_TEX(uv,_StatueTex));
			float grey = dot(col.rgb, float3(0.299, 0.587, 0.114));
			col.rgb = lerp(col.rgb, float3(grey, grey, grey), _StatueDegree);
			col.rgb *= lerp(fixed3(1,1,1), colStatue.rgb, _StatueDegree) * lerp(1,_StatueBrightness, _StatueDegree);
			return col;
		}
	ENDCG
	...
	SubShader {
		...
		pass
		{
			...
			{
				float4 _fCol = _OutlineColor * _Color;
				_fCol.a = _Color.a;
				return _fCol;								
			}
